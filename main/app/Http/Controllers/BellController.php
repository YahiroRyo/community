...
***REMOVED***
    public function getBells(Request $request) ***REMOVED***
        // $request->uid
        // $request->take
        // $request->gotNum
        if (ctype_digit(strval($request->take)) && (ctype_digit(strval($request->gotNum)) || !$request->gotNum)) ***REMOVED***
            ...
            $bells = Bell::where('user_id', $userId)->take($take + $gotNum)->get(['id', 'type', 'user_id']);
            for ($i = $gotNum; $i < count($bells); $i++) ***REMOVED***
                if ($bells[$i]['type'] === 1) ***REMOVED***
                    $bells[$i]['dataForType'] = CanIJoinCommunity::where('bell_id', $bells[$i]['id'])->with(['community' => function($query) ***REMOVED***
